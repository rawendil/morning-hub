...
test('dashboard has deferred props for todays_tasks blocks with configured connections', function () {
    Http::fake([
    ...
    $block = RoutineBlock::factory()->for($user)->create([
        'type' => BlockType::TodaysTasks,
        'sort_order' => 0,
        'config' => ['connection_ids' => [$connection->id]],
    ]);
    ...
    $this->actingAs($user)
        ...
        ->assertInertia(fn ($page) => $page
            ->component('Dashboard')
            ->has('blocks', 1)
            ->missing("todays_tasks_{$block->id}")
            ->loadDeferredProps(fn ($reload) => $reload
                ->has("todays_tasks_{$block->id}")
                ->where("todays_tasks_{$block->id}.error", null)
                ->has("todays_tasks_{$block->id}.groups", 1)
                ->where("todays_tasks_{$block->id}.groups.0.connectionId", $connection->id)
            )
        );
});

test('todays_tasks block without connection_ids has no deferred prop', function () {
    $user = User::factory()->create();
    $block = RoutineBlock::factory()->for($user)->create([
        'type' => BlockType::TodaysTasks,
        'sort_order' => 0,
...
test('todays_tasks deferred prop passes due_date and assignee filters to clickup api', function () {
    Http::fake([
        'https://api.clickup.com/api/v2/user' => Http::response([
            'user' => ['id' => 999, 'username' => 'testuser', 'email' => 'test@test.com'],
        ], 200),
    ...
    Http::assertSent(function ($request) {
        return str_contains($request->url(), '/list/list-1/task')
            && str_contains($request->url(), 'due_date_gt')
            && str_contains($request->url(), 'due_date_lt')
            && str_contains($request->url(), 'assignees');
    });
});
